...
function SkipCard({ skip, selected, onSelect }) {
  return (
    <div
      className={`relative bg-gray-800 rounded-xl shadow-lg p-4 flex flex-col md:flex-row items-center w-full border-2 transition-all duration-200 ${
        selected === skip.id
    ...
    >
      ...
      <div className='relative w-24 h-24 md:w-32 md:h-32 bg-gray-700 rounded-lg flex-shrink-0 mr-0 md:mr-4 mb-4 md:mb-0'>
        ...
        />
        <span className='absolute top-2 right-2 bg-blue-600 text-white text-xs px-1.5 py-0.5 rounded-full font-bold shadow-md'>
          {skip.size} Yards
        </span>
      </div>
